...
public sealed class JsonConfigurationService : IConfigurationService, IDisposable
{
    ...
    public void SaveConfiguration()
    {
        ...
        try
        {
            ...
            var content = JsonSerializer.Serialize(configuration, typeof(Configuration), ConfigurationSourceGenerationContext.Default);
    ...
    private Configuration LoadConfiguration()
    {
        try
        {
            ...
            return (Configuration)JsonSerializer.Deserialize(fileStream, typeof(Configuration), ConfigurationSourceGenerationContext.Default) ?? new Configuration();
        }
        catch (Exception exception)
        {
            throw new ServiceException(ExceptionMessage, exception);
    ...
    private void CreateConfigurationFileIfNeeded()
    {
        ...
        var content = JsonSerializer.Serialize(new Configuration(), typeof(Configuration), ConfigurationSourceGenerationContext.Default);
        File.WriteAllText(filePath, content);
    }
}